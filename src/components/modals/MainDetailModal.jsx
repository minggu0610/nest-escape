...
import { 
  ...
  CornerDownRight, Share2, BarChart3, Eye 
} from 'lucide-react';
import { 
...
export const MainDetailModal = ({ 
  ...
  issuedDocs, setIssuedDocs, onOpenAlert, userData 
}) => {
  ...
  const handleIssueAllDocs = () => {
    ...
    alert('필요한 서류가 모두 발급(연동)되어 서류함에 저장되었습니다. 이제 바로 열람이 가능합니다.');
  };
        ...
        <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} className="bg-white w-full max-w-xl rounded-[2rem] overflow-hidden shadow-2xl flex flex-col max-h-[85vh]" onClick={e => e.stopPropagation()}>
          ...
          <div className="p-7 overflow-y-auto space-y-6 flex-1 bg-gray-50/30">
            ...
            <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm">
              ...
              {policy.isPeerPopular && (
                <div className="mt-4 p-3 bg-blue-50 rounded-xl border border-blue-100 flex items-center gap-2">
                  <span className="text-[10px] font-black text-blue-600 bg-white px-1.5 py-0.5 rounded shadow-sm">또래 인기</span>
                  <p className="text-[10px] text-blue-500 font-bold">비슷한 연령대의 유저들이 가장 많이 확인한 정책입니다.</p>
                </div>
              )}
            </div>
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {policy.documents.map((doc, i) => (
                      <div key={i} className={cn("flex items-center justify-between p-2.5 rounded-lg border text-[11px] font-bold transition-all", issuedDocs.includes(doc) ? "bg-green-50 border-green-100 text-green-700" : "bg-gray-50 border-gray-100 text-gray-400")}>
                        ...
                        {issuedDocs.includes(doc) ? (
                          <button 
                            onClick={(e) => { e.stopPropagation(); setViewingDoc(doc); }}
                            className="flex items-center gap-1 px-2 py-0.5 bg-white rounded border border-green-200 text-green-600 hover:bg-green-500 hover:text-white transition-all shadow-sm"
                          >
                            <Eye size={10}/> 열람
                          </button>
                        ) : <span className="text-[9px] opacity-60">연동전</span>}
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="p-5 bg-white border-t border-gray-100 flex gap-2 flex-shrink-0">
            <button onClick={openOriginalNotice} className="flex-1 py-3.5 bg-gray-100 text-gray-700 text-sm font-black rounded-xl hover:bg-gray-200 transition-all flex items-center justify-center gap-2 shadow-sm border border-gray-200">
